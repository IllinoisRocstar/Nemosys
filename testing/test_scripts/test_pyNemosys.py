...
class TestPyNemosys(unittest.TestCase):
    ...
    #     self.datadir_path = os.path.join(self.CMAKE_CURRENT_SOURCE_DIR,
    #                                      'test_data/test_pyNemosys/')
    ...
    def testMeshBase(self):
        ...
        array_names = [str(i) for i in
                       inputjson['Transfer Options']['Array Names']]
        ...
        for name in array_names:
            print(name)
        transfer = TransferDriver.CreateTransferObject(source, target,
                                                       method_name)
        ...
        self.assertEqual(diffMesh(meshBase.Create(gold_output_file),
                                  meshBase.Create(output_file)), 0)
    ...
    def testTransferDriver(self):
        ...
        array_names = [str(i) for i in
                       inputjson['Transfer Options']['Array Names']]
        output_file = inputjson['Mesh File Options']['Output Mesh File']
        check_quality = inputjson['Transfer Options'][
                            'Check Transfer Quality'] in ['true', 'True']
        gold_output_file = 'gold_transfer_test.vtu'
        ...
        driver = TransferDriver(
            TransferDriver.Files(source_file, target_file, output_file), opts)
        driver.execute()
        self.assertEqual(diffMesh(meshBase.Create(gold_output_file),
                                  meshBase.Create(output_file)), 0)
    ...
    def testRefineDriver_value(self):
        ...
        print(frameinfo.filename + '-' + str(frameinfo.lineno))
        import pyNemosys
        if not hasattr(pyNemosys, 'SizeFieldRefineDriver'):
            print('No SizeFieldRefineDriver. Nothing to test')
            return
        ...
        array_name = inputjson["Refinement Options"]["Array Name"]
        dev_mult = inputjson["Refinement Options"]["StdDev Multiplier"]
        max_is_min = inputjson["Refinement Options"][
                         "Max Is Min for Scaling"] in [1, 'true', 'True']
        transfer_data = inputjson["Refinement Options"][
                            "Transfer Data"] in [1, 'true', 'True']
        print(max_is_min)

        gold_output_file = 'gold_refined_beam_value.vtu'

        opts = SizeFieldRefineDriver.Opts(
            SizeFieldRefineDriver.Opts.Method.GRADIENT
            if method == "gradient"
            else SizeFieldRefineDriver.Opts.Method.VALUE,
            array_name, dev_mult, max_is_min, transfer_data)

        refdrvobj = SizeFieldRefineDriver(
            SizeFieldRefineDriver.Files(_mesh, ofname), opts)
        refdrvobj.execute()
        # self.assertEqual(diffMesh(meshBase.Create(gold_output_file),
        #                           meshBase.Create(ofname)), 0)

    def testRefineDriver_uniform(self):
        frameinfo = getframeinfo(currentframe())
        print(frameinfo.filename + '-' + str(frameinfo.lineno))
        import pyNemosys
        if not hasattr(pyNemosys, 'UniformRefineDriver'):
            print('No UniformRefineDriver. Nothing to test')
            return
        ...
        transfer_data = bool(inputjson["Refinement Options"]["Transfer Data"])

        gold_output_file = 'gold_refined_beam_uniform.vtu'

        refdrvobj = UniformRefineDriver(
            UniformRefineDriver.Files(_mesh, ofname),
            UniformRefineDriver.Opts(transfer_data, edgescale))
        refdrvobj.execute()
        self.assertEqual(diffMesh(meshBase.Create(gold_output_file),
                                  meshBase.Create(ofname)), 0)
    ...
    def testMeshGenDriver(self):
        ...
        if not hasattr(pyNemosys, "NetgenMeshGenDriver"):
            ...
            return
        from pyNemosys import NetgenMeshGenDriver, netgenParams, diffMesh, \
            meshBase
        ...
        for param in ["uselocalh", "maxh", "fineness", "grading",
                      "elementsperedge", "elementspercurve",
                      "closeedgeenable", "closeedgefact", "second_order",
                      "meshsize_filename", "quad_dominated",
                      "optvolmeshenable", "optsteps_2d", "optsteps_3d",
                      "invert_tets", "invert_trigs", "check_overlap",
                      "check_overlapping_boundary", "refine_with_geom",
                      "refine_without_geom"]:
            if param in params_json:
                setattr(params, param, params_json[param])

        mshgndrvobj = NetgenMeshGenDriver(
            NetgenMeshGenDriver.Files(in_file, output_file), params)
        mshgndrvobj.execute()

        ref_mesh = meshBase.Create(gold_output_file)
        new_mesh = meshBase.Create(output_file)
        ...
        refpoints = ref_mesh.getNumberOfPoints()
        refcells = ref_mesh.getNumberOfCells()
        newpoints = new_mesh.getNumberOfPoints()
        newcells = new_mesh.getNumberOfCells()

        self.assertEqual(
            refpoints - refpoints / divisor
            <= newpoints
            <= refpoints + refpoints / divisor
            and refcells - refcells / divisor
            <= newcells
            <= refcells + refcells / divisor,
            True)
    ...
    def testMeshQualityDriver(self):
        ...
        qualdrvobj = CheckMeshQualDriver(
            CheckMeshQualDriver.Files(_mesh, ofname))
        qualdrvobj.execute()
            ...
            for lineNum in range(7):
                ...
                if lineNum > 2:
                    for valNum in range(6):
                        if valNum > 0:
                            if not abs(float(line1[valNum]) - float(
                                    line2[valNum])) <= epsilon:
                                return False
