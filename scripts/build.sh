...
mkdir build
cd build
CMAKE_PREFIX_PATH=$NEMOSYS_DEPS_INSTALL_PATH/madlib:$NEMOSYS_DEPS_INSTALL_PATH/gmsh:$NEMOSYS_DEPS_INSTALL_PATH/cgns cmake -DENABLE_PYTHON_BINDINGS=ON -DCMAKE_INSTALL_PREFIX=$NEMOSYS_DEPS_INSTALL_PATH -DENABLE_BUILD_UTILS=ON .. 
make -j
make install
