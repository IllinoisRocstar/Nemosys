...
#		./ build.sh PATH_TO_NEMOSYS PATH_TO_NEMOSYS_TPLS_TARBALL PATH_TO_INSTALL_DIR PATH_TO_HDF5_INC PATH_TO_HDF5_LIB
...
mkdir build
cd build
cmake -DCGNS_ENABLE_HDF5=ON -DCGNS_BUILD_CGNSTOOLS=ON -DCMAKE_INSTALL_PREFIX=$NEMOSYS_DEPS_INSTALL_PATH/cgns -DHDF5_INCLUDE_PATH=${HDF5_INC} -DHDF5_LIBRARY=${HDF5_LIB} ..
make -j${num_threads}
make install
