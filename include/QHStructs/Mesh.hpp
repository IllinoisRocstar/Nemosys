...
template <typename T> class NEMOSYS_EXPORT MeshBuilder {
 public:
  struct HalfEdge {
    ...
    void disable() { m_endVertex = std::numeric_limits<IndexType>::max(); }
  ...
  struct Face {
    ...
    // Bit for each half edge assigned to this face, each being 0 or 1
    // depending on whether the edge belongs to horizon edge
    ...
    void disable() { m_he = std::numeric_limits<IndexType>::max(); }
    ...
    Face(const Face &fi) = default;
    Face &operator=(const Face &fi) = default;
    /*
    Face(const Face& fi)
    {
      m_he = fi.m_he;
      m_P = fi.m_P;
      m_mostDistantPointDist = fi.m_mostDistantPointDist;
      m_mostDistantPoint = fi.m_mostDistantPoint;
      m_visibilityCheckedOnIteration = fi.m_visibilityCheckedOnIteration;
      m_isVisibleFaceOnCurrentIteration = fi.m_isVisibleFaceOnCurrentIteration;
      m_inFaceStack = fi.m_inFaceStack;
      // Bit for each half edge assigned to this face, each being 0 or 1
      // depending on whether the edge belongs to horizon edge
      m_horizonEdgesOnCurrentIteration = fi.m_horizonEdgesOnCurrentIteration;
      m_pointsOnPositiveSide = std::move(fi.m_pointsOnPositiveSide);
    }
    Face& operator=(const Face& fi)
    {
        return (Face(fi));
    }
    */
  ...
  // we do not actually remove them from the container vectors.
  // insted, they are marked as disabled which means that the indices
  // can be reused when we need to add new faces and half edges to the mesh.
  ...
  // mark a face as disabled and return a pointer to the points that
  // were on the positive of it.
  ...
  // create a mesh with initial tetrahedron ABCD. Dot product of AB with
  // the normal of triangle ABC should be negative.
  ...
  std::array<IndexType, 2> getVertexIndicesOfHalfEdge(
