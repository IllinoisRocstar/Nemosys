...
def Compute3DPolyFit(power, x, y, z, FDummy, G):
    ...
    print "Time for building kronprod full: ", (end-start)*1000
...
#X, Y = np.meshgrid(x, y)
#Z = G(X,Y) #F(X)*F(Y)
#result = Compute2DPolyFit(2,x,y,F,G)
#
#
#
#fig = plt.figure()
...
#
## Plot the surface.
##surf = ax.plot_surface(X, Y, np.reshape(result[0],(200,200)), cmap=cm.coolwarm,
##                       linewidth=0, antialiased=False)
##surf = ax.plot_surface(X, Y, Z, cmap=cm.coolwarm,
##                       linewidth=0, antialiased=False)
#
#surf = ax.plot_surface(X, Y, np.reshape(result[1],(200,200)), cmap=cm.coolwarm,
#                       linewidth=0, antialiased=False)
#
#
