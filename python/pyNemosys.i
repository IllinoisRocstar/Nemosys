...
class meshBase {
  ...
  public:
    ...
    static meshBase *
    Create(vtkSmartPointer<vtkDataSet> other, std::string newname);
    ...
    static meshBase *
    extractSelectedCells(meshBase *mesh, const std::vector<int> &cellIds);
    ...
    void
    writeMSH(std::ofstream &outputStream, std::string pointOrCell, int arrayID);
...
class vtkMesh : public meshBase {
  ...
  public:
    ...
    void setPointDataArray(const char *name,
                           const std::vector<std::vector<double>> &data);
    // set cell data (numComponents per cell determined by dim of data[0])
    void setCellDataArray(const char *name,
                          const std::vector<std::vector<double>> &data);
