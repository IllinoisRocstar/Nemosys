...
{
 // cleaning up
 for (int ic=0; ic<myCgObjs.size(); ic++)
   delete myCgObjs[ic];
}
...
{
  if (indx < cgFNames.size())
    return(myCgObjs[indx]->getBaseItrName());
  return("INVALID");
}
...
{
  if (indx < cgFNames.size())
    return(myCgObjs[indx]->getNTStep());
  return(-1);
}
...
{
  if (indx < cgFNames.size())
    return(myCgObjs[indx]->getTimeStep());
  return(-1);
}
...
std::vector<double> rocstarCgns::getZoneCoords(cgnsAnalyzer* cgObj, int zoneIdx, int dim)
{
