...
/// @brief Implements a command-line interface for the Nemosys tests.
///
...
/// usable and should be done for every new program in %Nemosys. 
///
#include "ComLine.H"
#include "TestNemosys.H"
#include "COMM.H"
...
namespace Nemosys{
  ...
  ///
  /// Drives the Nemosys::TestObject. 
  /// 
  /// @param argc number of string command line tokens
  /// @param argv string command line tokens
  /// @returns 0 if successful, 1 otherwise
  ///
  /// Drives the Nemosys::TestObject, which should encapsulate
  /// all the tests for the Nemosys namespace (and thus the project).
  /// 
  /// Command line documentation:
  ///
  ///           irproject_test [-h] [-v [level] -o <filename> -l <filename> -n <TestName> ] 
  ///
  ///           -h,--help
  ...
  int ParallelTest(int argc,char *argv[])
  {
    ...
    Nemosys::CommType communicator(&argc,&argv);
    int rank  = communicator.Rank();
    int nproc = communicator.Size();
    bool do_stdout = !rank;

    // This line creates the Nemosys::TestComLine object and passes in 
    ...
    if(!comline.GetOption("help").empty()){
      ...
      if(do_stdout){
        ...
        if(verblevel > 1)
          std::cout << "Nemosys::ParallelTest: Exiting test function (success)" 
                    << std::endl;
      }
    ...
    if(clerr){
      if(do_stdout){
        ...
        if(verblevel > 2)
          std::cout << "Nemosys::ParallelTest: Exiting test function (fail)" << std::endl;
      }
      communicator.SetExit(1);
    }
    if(communicator.Check())
    ...
    if(!OutFileName.empty()){
      if(Out){
        ...
        if(!Ouf){
          std::cout << "Nemosys::ParallelTest> Error: Could not open output file, " 
    ...
    if(verblevel > 1 && Out)
      *Out << "Nemosys::ParallelTest: Entering test function" << std::endl;
    
    // Make an instance of the Nemosys testing object, Nemosys::ParallelTestingObject
    Nemosys::ParallelTestingObject<Nemosys::CommType,Nemosys::TestResults> test(communicator);
    // Make an instance of the Nemosys results object, Nemosys::TestResults
    Nemosys::TestResults results;
    ...
    // run the listed tests.
      ...
      if(!ListInf){
        if(Out)
          *Out << "Nemosys::ParallelTest> Error: Could not open list of tests in file " 
      ...
      // This call runs all the tests for the Nemosys namespace.
    ...
    if((verblevel > 1) && Out)
      *Out << "Nemosys::ParallelTest: Exiting test function (success)" << std::endl;
...
{
  return(Nemosys::ParallelTest(argc,argv));
}
