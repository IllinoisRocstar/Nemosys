...
// class constructor
rocPack::rocPack(
    const std::string &fname, const std::string &outName) {
...
void rocPack::removeBoundaryVolumes(){
  removeBoundaryPacks = true;
}

void rocPack::setPeriodicGeometry(){
  enablePeriodicity = true;
}

void rocPack::setPeriodicMesh(){
  periodic3D = true;
}
...
void rocPack::enablePhysicalGrps(){
  enablePhysGrp = true;
}

void rocPack::enableTwoPhysGrps(){
  just2Physgrps = true;
}

void rocPack::enableSurfacePatches(){
...
void rocPack::rocParser() {
  ...
  if (rocOut.is_open()) {
    ...
    if (periodic3D == true && enablePeriodicity == false) {
      std::cerr << "Periodicity needed if using periodic meshing!"
                << std::endl << "Please enable SetPeriodicity option to"
    ...
    udfTranslate[0] = xUDF; udfTranslate[1] = yUDF; udfTranslate[2] = zUDF;
...
void rocPack::geomToSurf()
{
  ...
  if (OutFile.find(".stl") != std::string::npos)
    ...
    geomToVTK(OutFile + ".msh");
      ...
      geomToMsh(OutFile);
  else if (OutFile.find(".exo") != std::string::npos) {
    // Something here to write mesh in ExodusII format
  } else {
    std::string stlFile = OutFile + ".stl";
    std::string vtkFile = OutFile + ".msh";
    std::string mshFile = OutFile + ".msh";
    geomToSTL(stlFile); geomToMsh(mshFile); geomToVTK(vtkFile);
  }
}
...
void rocPack::geomToPeriodic3D() {
  ...
  if (just2Physgrps) {
    ...
    gmsh::model::getEntitiesForPhysicalGroup(3,surroundingGrp,surrondingEntities);
...
void rocPack::geomToVTK(const std::string &writeFile) {
  ...
  // gmsh::write(writeFile);
  geomToMsh(writeFile);
  meshBase *mb = meshBase::exportGmshToVtk(writeFile);
  mb->write(writeFile);
...
void rocPack::makePeriodic(const bool rmbPacks) {
  if (rmbPacks == false) {
    ...
    xTranslate.push_back(Xdim); yTranslate.push_back(0); zTranslate.push_back(0);
    xTranslate.push_back(Xdim); yTranslate.push_back(0); zTranslate.push_back(Zdim);
    xTranslate.push_back(Xdim); yTranslate.push_back(0); zTranslate.push_back(-Zdim);
    ...
    xTranslate.push_back(0); yTranslate.push_back(Ydim); zTranslate.push_back(0);
    xTranslate.push_back(0); yTranslate.push_back(Ydim); zTranslate.push_back(Zdim);
    xTranslate.push_back(0); yTranslate.push_back(Ydim); zTranslate.push_back(-Zdim);
    ...
    xTranslate.push_back(Xdim); yTranslate.push_back(Ydim); zTranslate.push_back(0);
    xTranslate.push_back(Xdim); yTranslate.push_back(Ydim); zTranslate.push_back(Zdim);
    xTranslate.push_back(Xdim); yTranslate.push_back(Ydim); zTranslate.push_back(-Zdim);
    ...
    xTranslate.push_back(0); yTranslate.push_back(0); zTranslate.push_back(Zdim);
    xTranslate.push_back(0); yTranslate.push_back(0); zTranslate.push_back(-Zdim);
...
std::vector<std::string> rocPack::Tokenize(
...
void rocPack::makeSphere(const int &n) {
  ...
  int newVol = gmsh::model::occ::addSphere(translateParams[n][0],
                                           translateParams[n][1],
                                           translateParams[n][2],
                                           sphereRad);
...
std::vector<double> rocPack::rotateByQuaternion(
...
void rocPack::mapPeriodicSurfaces(
  ...
  for (int i=0; i<tagsAllvols.size(); i++)
  {
    ...
    curveTagsAll.erase( unique( curveTagsAll.begin(), curveTagsAll.end() ), curveTagsAll.end() );
  ...
  gmsh::model::getEntitiesInBoundingBox(
    Xdim+boxPt[0]-tol, boxPt[1]-tol, boxPt[2]-tol, Xdim+boxPt[0]+tol,
    Ydim+boxPt[1]+tol, Zdim+boxPt[2]+tol, entityRight, 2);

  // Up
  std::vector<std::pair<int,int>> entityUp;
  gmsh::model::getEntitiesInBoundingBox(
    boxPt[0]-tol, Ydim+boxPt[1]-tol, boxPt[2]-tol, Xdim+boxPt[0]+tol,
    Ydim+boxPt[1]+tol, Zdim+boxPt[2]+tol, entityUp, 2);
  ...
  gmsh::model::getEntitiesInBoundingBox(
    boxPt[0]-tol, boxPt[1]-tol, Zdim+boxPt[2]-tol, Xdim+boxPt[0]+tol,
    Ydim+boxPt[1]+tol, Zdim+boxPt[2]+tol, entityFront, 2);
  ...
  affineTransform[0][0] = 1; affineTransform[0][1] = 0;
  affineTransform[0][2] = 0; affineTransform[0][3] = xT;
  affineTransform[0][4] = 0; affineTransform[0][5] = 1;
  affineTransform[0][6] = 0; affineTransform[0][7] = 0;
  affineTransform[0][8] = 0; affineTransform[0][9] = 0;
  affineTransform[0][10] = 1; affineTransform[0][11] = 0;
  affineTransform[0][12] = 0; affineTransform[0][13] = 0;
  affineTransform[0][14] = 0; affineTransform[0][15] = 1;

  affineTransform[1][0] = 1; affineTransform[1][1] = 0;
  affineTransform[1][2] = 0; affineTransform[1][3] = 0;
  affineTransform[1][4] = 0; affineTransform[1][5] = 1;
  affineTransform[1][6] = 0; affineTransform[1][7] = yT;
  affineTransform[1][8] = 0; affineTransform[1][9] = 0;
  affineTransform[1][10] = 1; affineTransform[1][11] = 0;
  affineTransform[1][12] = 0; affineTransform[1][13] = 0;
  affineTransform[1][14] = 0; affineTransform[1][15] = 1;

  affineTransform[2][0] = 1; affineTransform[2][1] = 0;
  affineTransform[2][2] = 0; affineTransform[2][3] = 0;
  affineTransform[2][4] = 0; affineTransform[2][5] = 1;
  affineTransform[2][6] = 0; affineTransform[2][7] = 0;
  affineTransform[2][8] = 0; affineTransform[2][9] = 0;
  affineTransform[2][10] = 1; affineTransform[2][11] = zT;
  affineTransform[2][12] = 0; affineTransform[2][13] = 0;
  affineTransform[2][14] = 0; affineTransform[2][15] = 1;
  ...
  for (int i=0; i<vertsOneSide.size(); i++) {
    ...
    for (int j=0; j<vertsOtherSide.size(); j++) {
      ...
      if (know == pointsOneSide.size())
        periodicSurfs.push_back(
          std::make_pair(vertsOneSide[i][0].first, vertsOtherSide[j][0].first));  
    }
  }
...
void rocPack::writePeriodicNodes() {
  ...
  for (int srf=0; srf<volSurfLinksX.size(); srf++) {
    ...
    gmsh::model::mesh::getPeriodicNodes(2,volSurfLinksX[srf].second,
                      masterSurfTag,slaveNodes,masterNodes,affineTransformData);
    ...
    if (srf == forSurfTestX && masterNodes.size() != 0) {
      ...
      if ((std::sqrt(Mcoords[0]*Mcoords[0])-std::sqrt(Scoords[0]*Scoords[0])) < 1e-05)
        if ((std::sqrt(Mcoords[1]*Mcoords[1])-std::sqrt(Scoords[1]*Scoords[1])) < 1e-05)
          if ((std::sqrt(Mcoords[2]*Mcoords[2])-std::sqrt(Scoords[2]*Scoords[2])) < 1e-05)
            matchingCoords = true;
          else
    ...
    for (int i=0; i<masterNodes.size(); i++)
      xDirectionNodes << "" << masterNodes[i] << ", " << slaveNodes[i] << std::endl;
  ...
  for (int srf=0; srf<volSurfLinksY.size(); srf++) {
    ...
    for (int i=0; i<masterNodes.size(); i++)
      yDirectionNodes << "" << masterNodes[i] << ", " << slaveNodes[i] << std::endl;
  ...
  for (int srf=0; srf<volSurfLinksZ.size(); srf++) {
    ...
    for (int i=0; i<masterNodes.size(); i++)
      zDirectionNodes << "" << masterNodes[i] << ", " << slaveNodes[i] << std::endl;
...
void rocPack::setRandomSurface(const int& surf) {
  randomSurfTest = surf;
}
...
void rocPack::performSmoothing() {
  ...
  for (int i=0; i<tagsSurfs.size(); i++) {
    gmsh::model::mesh::setSmoothing(tagsSurfs[i].first,
                                    tagsSurfs[i].second,
                                    smoothingIter);
  }
  gmsh::model::occ::synchronize();
...
void rocPack::createCohesiveElements(
  ...
  /*// Take intersection of geomNodes and periodic nodes to rule out surface nodes
  for (int j=0; j<geomsNodeTags.size(); j++)
  {
    std::vector<double> getPt = std::vector<double>(3);
    dataSetSurr->GetPoint(geomsNodeTags[j], &getPt[0]);
  ...
  for (int i=0; i<(interfaceNodes.size()); i++) {
    ...
    for( cellNum=0; cellNum<nCells; cellNum++ ) {
      ...
      if (grpIds[tmpCellID[cellNum]] == 2)
        cellPointPair.push_back(std::make_pair(tmpCellID[cellNum],interfaceNodes[i]));
    }
  }

  sort( cellPointPair.begin(), cellPointPair.end() );
  cellPointPair.erase( unique( cellPointPair.begin(), cellPointPair.end() ), cellPointPair.end() );
  ...
  UniqueCells.erase( unique( UniqueCells.begin(), UniqueCells.end() ), UniqueCells.end() );
