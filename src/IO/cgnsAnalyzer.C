...
void solutionData::appendData(const vecSlnType &inBuff, int inNData,
                              int inNDim) {
  // sanity check
  if (!slnData.empty()) {
    if (nDim != inNDim || inNDim == 0) {
      std::cerr << "Incompatible data size can not be appended."
                << "Current nDim = " << nDim << " Requested nDim = " << inNDim
                << std::endl;
      return;
    }
  } else {
...
void solutionData::getData(vecSlnType &outBuff, int &outNData,
                           int &outNDim) const {
  // deep copy data
...
int cgnsAnalyzer::getIndexFile() { return indexFile; }

int cgnsAnalyzer::getIndexBase() { return indexBase; }

int cgnsAnalyzer::getCellDim() { return cellDim; }

std::string cgnsAnalyzer::getFileName() { return cgFileName; }

std::string cgnsAnalyzer::getBaseName() { return baseName; }

std::string cgnsAnalyzer::getZoneName() { return zoneName; }

std::string cgnsAnalyzer::getZoneName(int iCgFile) {
  return zoneNames[iCgFile];
}

std::string cgnsAnalyzer::getSectionName() { return sectionName; }

std::string cgnsAnalyzer::getBaseItrName() { return baseItrName; }

int cgnsAnalyzer::getNZone() { return nZone; }

int cgnsAnalyzer::getNTStep() { return nTStep; }

std::string cgnsAnalyzer::getZoneItrName() { return zoneItrName; }

std::string cgnsAnalyzer::getGridCrdPntr() { return gridCrdPntr; }

std::string cgnsAnalyzer::getSolutionPntr() { return flowSlnPntr; }

int cgnsAnalyzer::getNVertex() { return nVertex; }

int cgnsAnalyzer::getNElement() { return nElem; }

int cgnsAnalyzer::getPhysDim() { return physDim; }

int cgnsAnalyzer::getElementType() { return sectionType; }

int cgnsAnalyzer::getNVrtxElem() { return nVrtxElem; }

double cgnsAnalyzer::getTimeStep() { return timeLabel; }

CGNS_ENUMT(MassUnits_t) cgnsAnalyzer::getMassUnit() { return massU; }

CGNS_ENUMT(LengthUnits_t) cgnsAnalyzer::getLengthUnit() { return lengthU; }

CGNS_ENUMT(TimeUnits_t) cgnsAnalyzer::getTimeUnit() { return timeU; }

CGNS_ENUMT(TemperatureUnits_t) cgnsAnalyzer::getTemperatureUnit() {
  return tempU;
}

CGNS_ENUMT(AngleUnits_t) cgnsAnalyzer::getAngleUnit() { return angleU; }

CGNS_ENUMT(ZoneType_t) cgnsAnalyzer::getZoneType() { return zoneType; }

bool cgnsAnalyzer::isStructured() { return !isUnstructured; }
...
double cgnsAnalyzer::getVrtXCrd(int vrtxId) { return xCrd[vrtxId]; }

std::vector<double> cgnsAnalyzer::getVrtXCrd() { return xCrd; }

double cgnsAnalyzer::getVrtYCrd(int vrtxId) { return yCrd[vrtxId]; }

std::vector<double> cgnsAnalyzer::getVrtYCrd() { return yCrd; }

double cgnsAnalyzer::getVrtZCrd(int vrtxId) { return zCrd[vrtxId]; }

std::vector<double> cgnsAnalyzer::getVrtZCrd() { return zCrd; }
...
void cgnsAnalyzer::getSectionConn(std::string secName,
                                  std::vector<cgsize_t> &conn, int &nElm) {
  std::vector<std::string> secNames;
  getSectionNames(secNames);
  auto it = std::find(secNames.begin(), secNames.end(), secName);
  if (it == secNames.end()) {
...
solution_type_t cgnsAnalyzer::getSolutionDataStitched(
    std::string sName, std::vector<double> &slnData, int &outNData,
    int &outNDim) {
  // load data if needed
  if (slnDataCont.empty()) loadSolutionDataContainer();
...
void cgnsAnalyzer::appendSolutionData(std::string sName,
                                      std::vector<double> &slnData,
                                      solution_type_t dt, int inNData,
                                      int inNDim) {
  // load data if needed
  if (slnDataCont.empty()) loadSolutionDataContainer();
...
void cgnsAnalyzer::getAppendedSolutionDataName(
    std::vector<std::string> &appSName) {
...
std::vector<CGNS_ENUMT(GridLocation_t)>
cgnsAnalyzer::getSolutionGridLocations() {
...
std::map<std::string, CGNS_ENUMT(GridLocation_t)>
cgnsAnalyzer::getSolutionNameLocMap() {
...
void cgnsAnalyzer::overwriteSolData(const std::string &fname,
                                    const std::string &ndeName, int slnIdx,
                                    CGNS_ENUMT(DataType_t) dt, void *data) {
