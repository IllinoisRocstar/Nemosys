/*
 * This is a copy of dataSetRegionSurfaceFilter, copyright Kitware, Inc,
 * ported from ParaView's VTKExtensions, with slight modifications to deal with
 * extracting curves from 2d meshes as well as maintaining the ID of the twin
...
class dataSetRegionSurfaceFilter::Internals {
 public:
  Internals() : NextRegion(0) { this->OldToNew[-1] = -1; };
  ~Internals() = default;
...
vtkStandardNewMacro(dataSetRegionSurfaceFilter)

//----------------------------------------------------------------------------
dataSetRegionSurfaceFilter::dataSetRegionSurfaceFilter() {
  this->RegionArray = 0;
  ...
  this->CellFaceIds = vtkIntArray::New();
  this->CellFaceIds->SetName("CellFaceIds");
  this->CellFaceIds->SetNumberOfComponents(1);
  this->TwinIds = vtkIdTypeArray::New();
  this->TwinIds->SetName("TwinIds");
  this->TwinIds->SetNumberOfComponents(1);
  this->Internal = new dataSetRegionSurfaceFilter::Internals;
...
//----------------------------------------------------------------------------
int dataSetRegionSurfaceFilter::FillInputPortInformation(int,
                                                         vtkInformation *info) {
...
//----------------------------------------------------------------------------
int dataSetRegionSurfaceFilter::RequestData(
  ...
  // assume all tets, and that the tets are small relative to the size of
  // the regions (absolute max number of faces in output would be
  ...
  this->TwinIds->Reset();
  this->TwinIds->Allocate(input->GetNumberOfCells());
  ...
  } else {  // get rid of point mapping information
    output->GetPointData()->RemoveArray("vtkOriginalPointIds");
  }
  output->GetCellData()->AddArray(this->TwinIds);

  return 1;
}

//----------------------------------------------------------------------------
int dataSetRegionSurfaceFilter::UnstructuredGridExecute(
  ...
  if (this->Dimension == 2) {
    ...
    auto cellIter = input->NewCellIterator();
    for (cellIter->InitTraversal(); !cellIter->IsDoneWithTraversal() && !abort;
      ...
      if (cellIter->GetCellDimension() == 2) {
        cellIter->GetCell(cell);
        for (i = 0; i < cell->GetNumberOfEdges(); ++i) {
          auto edge = cell->GetEdge(i);
          this->InsertEdgeInHash(edge->PointIds->GetId(0),
                                 edge->PointIds->GetId(1), cellId, i);
        }
      }
    }
    cellIter->Delete();
    ...
    for (cellId = 0; cellId < numCells && !abort; cellId++) {
      ...
      if (cellType == VTK_VERTEX || cellType == VTK_POLY_VERTEX) {
        ...
        this->InsertPolygonInHash(&ids[6], 6, cellId);
                 ...
                 cellType == VTK_BIQUADRATIC_QUAD) {  // save 2D cells for
                                                      // second pass
        flag2D = 1;
      } else
      ...
      // and higher order cells.
      {
        ...
        else  // process nonlinear cells via triangulation
        {
          if (cell->GetCellDimension() == 1) {
            ...
            vtkWarningMacro(<< "2-D nonlinear cells must be processed with all "
                               "other 2-D cells.");
          } else  // 3D nonlinear cell
          {
            ...
            for (j = 0; j < numFaces; j++) {
              ...
              if (cellIds->GetNumberOfIds() <= 0) {
                // FIXME: Face could not be consistent.
                  ...
                  switch (face->GetCellType()) {
                    case VTK_QUADRATIC_TRIANGLE:
                      this->InsertTriInHash(
                          face->PointIds->GetId(0), face->PointIds->GetId(1),
                          face->PointIds->GetId(2), cellId, j);
                      break;
                    case VTK_QUADRATIC_QUAD:
                    case VTK_BIQUADRATIC_QUAD:
                    case VTK_QUADRATIC_LINEAR_QUAD:
                      this->InsertQuadInHash(
                          face->PointIds->GetId(0), face->PointIds->GetId(1),
                          face->PointIds->GetId(2), face->PointIds->GetId(3),
                          cellId, j);
                      break;
                    default:
    ...
    // to the hashes.  Alternatively, the higher order 2d cells could be
    // handled in the following loop.
    ...
    for (cellId = 0; cellId < numCells && !abort && flag2D; cellId++) {
      ...
      // If we have a quadratic face and our subdivision level is zero, just
      // treat it as a linear cell.  This should work so long as the first
      // points of the quadratic cell correspond to all those of the
      // equivalent linear cell (which all the current definitions do).
      if (this->NonlinearSubdivisionLevel < 1) {
        switch (cellType) {
          case VTK_QUADRATIC_TRIANGLE:
      ...
      // A couple of common cases to see if things go faster.
      if (cellType == VTK_PIXEL) {  // Do we really want to insert the 2D
                                    // cells into a hash?
        pts->Reset();
        pts->InsertId(0,
                      this->GetOutputPointId(ids[0], input, newPts, outputPD));
        pts->InsertId(1,
                      this->GetOutputPointId(ids[1], input, newPts, outputPD));
        pts->InsertId(2,
                      this->GetOutputPointId(ids[3], input, newPts, outputPD));
        pts->InsertId(3,
                      this->GetOutputPointId(ids[2], input, newPts, outputPD));
        newPolys->InsertNextCell(pts);
        this->RecordOrigCellId(this->NumberOfNewCells, cellId);
        outputCD->CopyData(cd, cellId, this->NumberOfNewCells++);
      } else if (cellType == VTK_POLYGON || cellType == VTK_TRIANGLE ||
                 cellType == VTK_QUAD) {
        pts->Reset();
        for (i = 0; i < numCellPts; i++) {
          inPtId = ids[i];
        ...
        // Change strips to triangles so we do not have to worry about
        // order.
        int toggle = 0;
        vtkIdType ptIds[3];
        // This check is not really necessary.  It was put here because of
        ...
        if (numCellPts > 1) {
          ...
          for (i = 2; i < numCellPts; ++i) {
            ...
            toggle = !toggle;
                 ...
                 cellType == VTK_QUADRATIC_LINEAR_QUAD) {
        // Note: we should not be here if this->NonlinearSubdivisionLevel is
        // less than 1.  See the check above.
        input->GetCell(cellId, cell);
        cell->Triangulate(0, pts, coords);
        // Copy the level 1 subdivision points (which also exist in the
        // input and can therefore just be copied over.  Note that the
        // output of Triangulate records triangles in pts where each 3
        // points defines a triangle.  We will keep this invariant and also
        // keep the same invariant in parametericCoords and outPts later.
        outPts->Reset();
        ...
        if (this->NonlinearSubdivisionLevel > 1) {
          // We are going to need parametric coordinates to further
          // subdivide.
          ...
          // Subdivide these triangles as many more times as necessary.
          // Remember that we have already done the first subdivision.
          for (j = 1; j < this->NonlinearSubdivisionLevel; j++) {
            ...
            for (i = 0; i < outPts->GetNumberOfIds(); i += 3) {
              // Hold the input point ids and parametric coordinates.  First
              // 3 indices are the original points.  Second three are the
              // midpoints in the edges (0,1), (1,2) and (2,0), respectively
              // (see comment below).
              vtkIdType inPts[6];
              ...
              for (k = 3; k < 6; k++) {
                ...
                inParamCoords[k][0] =
                    0.5 * (inParamCoords[pt1][0] + inParamCoords[pt2][0]);
                inParamCoords[k][1] =
                    0.5 * (inParamCoords[pt1][1] + inParamCoords[pt2][1]);
                inParamCoords[k][2] =
                    0.5 * (inParamCoords[pt1][2] + inParamCoords[pt2][2]);
                inPts[k] =
                    GetInterpolatedPointId(inPts[pt1], inPts[pt2], input, cell,
                                           inParamCoords[k], newPts, outputPD);
              }
              //       * 0
            ...
            // Now that we have recorded the subdivided triangles in outPts2
            // and parametricCoords2, swap them with outPts and
            // parametricCoords to make them the current ones.
    ...
    vtkErrorMacro("Dimension must be set to 2 or 3");
  }

  // Now transfer geometry from hash to output (in 3d, only triangles and
  // quads).
  this->InitQuadHashTraversal();
  vtkIntArray *outRegionArray = NULL;
  if (this->RegionArrayName) {
    outRegionArray =
        vtkIntArray::SafeDownCast(outputCD->GetArray(this->RegionArrayName));
  }
  while ((q = this->GetNextVisibleQuadFromHash())) {
    // handle all polys
    for (i = 0; i < q->numPts; i++) {
      q->ptArray[i] =
          this->GetOutputPointId(q->ptArray[i], input, newPts, outputPD);
    }
    if (this->Dimension == 2) {
      newLines->InsertNextCell(q->numPts, q->ptArray);
    ...
    if (outRegionArray) {
      outRegionArray->SetValue(this->NumberOfNewCells,
                               this->Internal->NextRegion);
    }
  ...
  if (outRegionArray) {
    ...
    vtkStringArray *inMaterialSpecs =
        vtkStringArray::SafeDownCast(input->GetFieldData()->GetAbstractArray(
    ...
    for (it = this->Internal->NewRegions.begin();
         it != this->Internal->NewRegions.end(); it++) {
      int index = it->second;
      ...
      if (inMaterialSpecs && outMaterialSpecs) {
        ...
        if (pid1 == -1 && inMaterialSpecs) {
          ...
          outMaterialSpecs->SetValue(index,
                                     inMaterialSpecs->GetValue(location));
        } else {
  ...
  newVerts->Delete();
  newVerts = NULL;
  if (this->Dimension == 2 || newLines->GetNumberOfCells() > 0) {
...
//----------------------------------------------------------------------------
void dataSetRegionSurfaceFilter::InsertQuadInHash(vtkIdType a, vtkIdType b,
  ...
  while (quad) {
    ...
    if (quad->numPts == 4 && c == quad->ptArray[2]) {
...

      {
        // We have a match.
        quad->SourceId = -1;
        // That is all we need to do.  Hide any quad shared by two or more
        // cells.
        return;
      }
    }
  ...
  // but using pointer math, so that we don't generate a warning about
  // accessing ptArray out of bounds
  ...
  *quadsFaceId = faceId;
  // Space for setting the index of the win.
  vtkIdType *quadsTwinId = quad->ptArray + quadRealNumPts + 2;
  *quadsTwinId = -1;
...
//----------------------------------------------------------------------------
void dataSetRegionSurfaceFilter::InsertTriInHash(vtkIdType a, vtkIdType b,
                                                 vtkIdType c,
                                                 vtkIdType sourceId,
                                                 vtkIdType faceId) {
  int tmp;
  ...
  // We can't put the second smnallest in b because it might change the
  // order of the verticies in the final triangle.
  ...
  while (quad) {
    ...
    if (quad->numPts == 3) {
      if (((b == quad->ptArray[1] && c == quad->ptArray[2]) ||
          ...
          (regionId == -1 ||
           regionId == *quadsRegionId))  // internal cells, but on a
                                         // material interface
      {
        // We have a match.
        quad->SourceId = -1;
        // That is all we need to do. Hide any tri shared by two or more
        // cells (that also are from same region)
        return;
      }
    }
    quad = *end;
...
//----------------------------------------------------------------------------
void dataSetRegionSurfaceFilter::InsertEdgeInHash(vtkIdType a, vtkIdType b,
                                                  vtkIdType sourceId,
                                                  vtkIdType faceId) {
  int tmp;
  vtkFastGeomQuad *quad, **end;
  ...
  // Look for existing edge in the hash;
  end = this->QuadHash + a;
  quad = *end;
  vtkIdType regionId = -1;
  if (this->RegionArray) {
  ...
  quad->ptArray[0] = a;
  quad->ptArray[1] = b;
  quad->ptArray[2] = regionId;
  quad->ptArray[3] = faceId;
  const int quadRealNumPts(2);
  quad->numPts = quadRealNumPts;
  vtkIdType *quadsTwinId = quad->ptArray + quadRealNumPts + 2;
  *quadsTwinId = -1;
  *end = quad;
...
//----------------------------------------------------------------------------
void dataSetRegionSurfaceFilter::RecordOrigCellId(vtkIdType destIndex,
                                                  vtkFastGeomQuad *quad) {
  this->OrigCellIds->InsertValue(destIndex, quad->SourceId);
  if ((this->Dimension == 2 && quad->numPts == 2) ||
      (this->Dimension == 3 && (quad->numPts == 3 || quad->numPts == 4))) {
    const vtkIdType *faceId = (quad->ptArray + (quad->numPts + 1));
    this->CellFaceIds->InsertValue(destIndex, *faceId);
    const vtkIdType *twinId = quad->ptArray + quad->numPts + 2;
    this->TwinIds->InsertValue(destIndex, *twinId);
    if (*twinId >= 0) {
      this->TwinIds->SetValue(*twinId, destIndex);
    } else if (*twinId != -1) {
      vtkErrorMacro(
          "Invalid twin index. quad possibly not created from "
          "InsertEdgeInHash, "
          "InsertTriInHash, or InsertQuadInHash");
    }
  } else {
    this->CellFaceIds->InsertValue(destIndex, -1);
    this->TwinIds->InsertValue(destIndex, -1);
  }
}

//----------------------------------------------------------------------------
void dataSetRegionSurfaceFilter::PrintSelf(ostream &os, vtkIndent indent) {
  this->Superclass::PrintSelf(os, indent);
}

//----------------------------------------------------------------------------
vtkFastGeomQuad *dataSetRegionSurfaceFilter::GetNextVisibleQuadFromHash() {
  if (!this->RegionArray) {
  ...
  while (quad == NULL || quad->SourceId == -1) {
    if (quad) {  // The quad must be hidden.  Move to the next.
      quad = quad->Next;
    } else {  // must be the end of the linked list.  Move to the next bin.
      this->QuadHashTraversalIndex += 1;
      if (this->QuadHashTraversalIndex >=
          this->QuadHashLength) {  // There are no more bins.
  ...
  if (!this->SingleSided) {
    ...
    while (quad2) {
      bool match = false;
      if (quad2->SourceId != -1 &&
          this->RegionArray->GetValue(quad2->SourceId) != mat1) {
        if (npts == 2) {
          if (quad2->numPts == 2 && quad->ptArray[1] == quad2->ptArray[1]) {
            match = true;
          }
        } else if (npts == 3) {
          if (quad2->numPts == 3 && ((quad->ptArray[1] == quad2->ptArray[1] &&
                                      quad->ptArray[2] == quad2->ptArray[2]) ||
                                     (quad->ptArray[1] == quad2->ptArray[2] &&
                                      quad->ptArray[2] == quad2->ptArray[1]))) {
            match = true;
          }
        } else if (npts == 4) {
          if (quad2->numPts == 4 && ((quad->ptArray[1] == quad2->ptArray[1] &&
                                      quad->ptArray[3] == quad2->ptArray[3]) ||
                                     (quad->ptArray[1] == quad2->ptArray[3] &&
                                      quad->ptArray[3] == quad2->ptArray[1]))) {
            match = true;
          }
        }
      }
      if (match) {
        vtkIdType *quad2TwinId = quad2->ptArray + quad2->numPts + 2;
        *quad2TwinId = this->NumberOfNewCells;
        break;
      } else {
        quad2 = quad2->Next;
      }
    ...
    if (this->Internal->NewRegions.find(p) ==
        this->Internal->NewRegions.end()) {
    ...
    while (quad2) {
      ...
      if (match) {
        ...
        if (mat2 > mat1) {
          // pick greater material to ensure a consistent ordering for
          // normals
          ...
          // Because we change quad->SourceId, we should also change
          // quad's FaceId
          vtkIdType *quadFaceId = quad->ptArray + (quad->numPts + 1);
          vtkIdType *quad2FaceId = quad2->ptArray + (quad2->numPts + 1);
          *quadFaceId = *quad2FaceId;
        }
