    ...
    int ParallelProgram::Run()
    {
      ...
      if(myn > 0){ // if there are points in this processor's domain
        try {
          ...
          my_tpi = Nemosys::TrapezoidQuadrature(f,a,b,myn);
          FunctionExit("TrapezoidQuadrature");
        } catch (...){
          StdOut("Numerical limits of Nemosys::TrapezoidQuadrature exceeded.");
          my_tpi = 0.0;
        }
      }
      double tpi = 0.0;
      ...
      if(myn > 0) {  // if there are points in this processor's domain
        try{
          ...
          my_mppi = Nemosys::MidPointQuadrature(f,a,b,myn);
          FunctionExit("MidPointQuadrature");
        } catch (...){
          StdOut("Numerical limits of Nemosys::MidPointQuadrature exceeded.");
          my_mppi = 0.0;
        }
      }
      double mppi = 0.0;
