...
RocRestartDriver::RocRestartDriver(
    const std::vector<std::string> &_fluidNamesRm,
    ...
    : fluidNamesRm(_fluidNamesRm),
      ifluidniNamesRm(_ifluidniNamesRm),
      ifluidnbNamesRm(_ifluidnbNamesRm),
      ifluidbNamesRm(_ifluidbNamesRm),
      fluidNamesLts(_fluidNamesLts),
      ifluidniNamesLts(_ifluidniNamesLts),
      ifluidnbNamesLts(_ifluidnbNamesLts),
      ifluidbNamesLts(_ifluidbNamesLts),
      burnNamesRm(_burnNamesRm),
      iBurnNamesRm(_iBurnNamesRm),
      burnNamesLts(_iBurnNamesLts),
      iBurnNamesLts(_iBurnNamesLts) {
...
void RocRestartDriver::stitchCGNS(const std::vector<std::string> &fnames,
                                  bool surf) {
  if (fnames.size()) {
    stitchers.push_back(
...
void RocRestartDriver::transferStitchedToPartCg(
  ...
  for (int i = 0; i < fluidRmMb.size(); ++i) {
    ...
    auto transfer = TransferDriver::CreateTransferObject(
        mbObjs[0], fluidRmMb[i].get(), transferType);
  ...
  for (int i = 0; i < burnRmMb.size(); ++i) {
    ...
    auto transfer = TransferDriver::CreateTransferObject(
        mbObjs[1], burnRmMb[i].get(), transferType);
  ...
  for (int i = 0; i < iBurnRmMb.size(); ++i) {
    ...
    auto transfer = TransferDriver::CreateTransferObject(
        mbObjs[2], iBurnRmMb[i].get(), transferType);
  ...
  for (int i = 0; i < ifluidNiRmMb.size(); ++i) {
    ...
    auto transfer = TransferDriver::CreateTransferObject(
        stitchedSurf, ifluidNiRmMb[i].get(), transferType);
  ...
  for (int i = 0; i < ifluidNbRmMb.size(); ++i) {
    ...
    auto transfer = TransferDriver::CreateTransferObject(
        stitchedSurf, ifluidNbRmMb[i].get(), transferType);
    transfer->run(stichedSurf->getNewArrayNames());
  ...
  for (int i = 0; i < ifluidBRmMb.size(); ++i) {
    ...
    auto transfer = TransferDriver::CreateTransferObject(
        stitchedSurf, ifluidBRmMb[i].get(), transferType);
    transfer->run(stichedSurf->getNewArrayNames());
...
cgVtPair RocRestartDriver::loadCGNS(const std::vector<std::string> &fnames,
                                    bool surf) {
...
RocRestartDriver *RocRestartDriver::readJSON(json inputjson) {
  ...
  std::string prepBurnDir =
      inputjson["Rocprep Remesh Burn Directory"].as<std::string>();
  std::string lastDir = inputjson["Rocout Last TS Directory"].as<std::string>();
  std::string lastBurnDir =
      inputjson["Rocout Last TS Burn Directory"].as<std::string>();
  ...
  std::vector<std::string> burnNamesRm(
      getCgFNames(prepBurnDir, "burn", base_tRm));
  std::vector<std::string> iBurnNamesRm(
      getCgFNames(prepBurnDir, "iburn", base_tRm));
  std::vector<std::string> ifluniNamesRm(
      getCgFNames(prepDir, "ifluid_ni", base_tRm));
  std::vector<std::string> iflunbNamesRm(
      getCgFNames(prepDir, "ifluid_nb", base_tRm));
  std::vector<std::string> iflubNamesRm(
      getCgFNames(prepDir, "ifluid_b", base_tRm));
  std::vector<std::string> fluNamesLts(getCgFNames(lastDir, "fluid", base_t));
  std::vector<std::string> burnNamesLts(
      getCgFNames(lastBurnDir, "burn", base_t));
  std::vector<std::string> iBurnNamesLts(
      getCgFNames(lastBurnDir, "iburn", base_t));
  std::vector<std::string> ifluniNamesLts(
      getCgFNames(lastDir, "ifluid_ni", base_t));
  std::vector<std::string> iflunbNamesLts(
      getCgFNames(lastDir, "ifluid_nb", base_t));
  std::vector<std::string> iflubNamesLts(
      getCgFNames(lastDir, "ifluid_b", base_t));

  RocRestartDriver *restartDriver = new RocRestartDriver(
      fluNamesRm, ifluniNamesRm, iflunbNamesRm, iflubNamesRm, fluNamesLts,
      ifluniNamesLts, iflunbNamesLts, iflubNamesLts, burnNamesRm, iBurnNamesRm,
      burnNamesLts, iBurnNamesLts);
