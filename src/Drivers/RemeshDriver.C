...
void backUpDir(const std::string &_cur_dir,
               const std::string &_bak_dir);
void rmDirContent(const boost::filesystem::path &_dir,
                  bool _skipDir);
bool expContains(const std::string &_name,
                 const std::string &_exp);
void rmFileName(const boost::filesystem::path &_dir,
                const std::string &_fn);
void cpFileName(const boost::filesystem::path &_src_dir,
                const boost::filesystem::path &_des_dir,
                const std::string &_fn,
                const std::string &_starts);
bool createDir(const boost::filesystem::path &directory);

RemeshDriver::RemeshDriver(std::vector<std::string> _fluidNames,
                           std::vector<std::string> _ifluidniNames,
                           std::vector<std::string> _ifluidnbNames,
                           std::vector<std::string> _ifluidbNames,
                           std::vector<std::string> _burnNames,
                           std::vector<std::string> _iBurnNames,
                           const jsoncons::json &remeshjson,
                           int _numPartitions,
                           const std::string &base_t,
                           int writeIntermediateFiles,
...
void RemeshDriver::remesh(const jsoncons::json &remeshjson)
{
  ...
  std::unique_ptr<meshBase> surf = std::unique_ptr<meshBase>
      (meshBase::Create(mbObjs[0]->extractSurface(),
                        prefixPath + "extractedSurface.vtp"));
...
RemeshDriver *RemeshDriver::readJSON(const jsoncons::json &inputjson)
{
  ...
  if (inputjson.has_key("Write To Temporary Directory"))
  {
    ...
    if (writeToTemporaryDirectory)
    {
      ...
      {
        std::cerr
            << "Specify Temporary RocfluMP Case Directory and"
  ...
  RemeshDriver *remeshdrvobj = new RemeshDriver(fluNames, ifluniNames,
                                                iflunbNames,
                                                iflubNames, burnNames,
                                                iBurnNames,
                                                remeshjson, numPartitions,
                                                base_t,
                                                writeIntermediateFiles,
                                                searchTolerance,
                                                case_name, surfacePatchTypes,
                                                rmsh_dir.string());
...
// recursive copy, will create destination folder (make sure it is removed first)
                ...
                const std::string &_starts)
{
       ...
       it != end_dir_it; ++it)
  {
    ...
    if (!expContains(fext, _fn)) continue;
    {
      ...
      boost::filesystem::path des_path(
          _des_dir.string() + "/" + it->path().filename().string());
