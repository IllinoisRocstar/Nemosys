...
SizeFieldBase *
SizeFieldBase::Create(meshBase *_mesh, const std::string &method, int arrayID,
                      double _dev_mult, bool _maxIsmin, double sizeFactor)
{
  if (method == "value")
  {
    auto *valsf = new ValSizeField(_mesh, arrayID, _dev_mult, _maxIsmin);
  ...
  {
    auto *gradsf = new GradSizeField(_mesh, arrayID, _dev_mult, _maxIsmin);
  ...
  {
    auto *z2errorsf = new Z2ErrorSizeField(_mesh, arrayID);
...
SizeFieldBase::CreateUnique(meshBase *_mesh, const std::string &method,
...
//  std::cout << "Size Factor = " << _sizeFactor << std::endl;
  return std::unique_ptr<SizeFieldBase>(
      SizeFieldBase::Create(_mesh, method, arrayID, _dev_mult, _maxIsmin,
                            _sizeFactor));
}
                               ...
                               bool _maxIsmin, const std::string &arrName)
{
  ...
  if (arrName !="Z2ErrorSF")
    da = mesh->getDataSet()->GetPointData()->GetArray(arrayID);
  // setting name of size field
  std::string array_name = mesh->getDataSet()->GetPointData()->GetArrayName(
      arrayID);
