...
bool
readLegacyVTKPoints(std::istream &meshStream,
                    std::string &line,
                    nemId_t &numPoints,
...
bool
readLegacyVTKCells(std::istream &meshStream,
                   std::string &line,
                   nemId_t &numCells,
    ...
    while (getline(meshStream, line))
    {
      if (!line.empty())
      {
        ...
        else
        {
          ...
          if (hasScalar != std::string::npos)
            numComponent = numComponent > 4 || numComponent < 1
                           ? 1
                           : numComponent;
...
void vtkMesh::inspectEdges(const std::string &ofname) const
{
  ...
  for (nemId_t i = 0; i < extractEdges->GetOutput()->GetNumberOfCells(); ++i)
  {
    ...
    double len = sqrt(pow(p1[0] - p2[0], 2)
                      + pow(p1[1] - p2[1], 2)
                      + pow(p1[2] - p2[2], 2));
...
void
vtkMesh::getPointDataArray(const std::string &name, std::vector<double> &data)
{
  int idx;
...
void
vtkMesh::getCellDataArray(const std::string &name, std::vector<double> &data)
{
  int idx;
