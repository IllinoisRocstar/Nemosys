...
PNTMesh::surfaceBCTag PNTMesh::bcTagNum(const std::string &_tag)
{
  std::string tag = _tag;
  nemAux::toLower(tag);
...
PNTMesh::elementType PNTMesh::elmTypeNum(const std::string &_tag)
{
  std::string tag = _tag;
  nemAux::toLower(tag);
...
// assumes all elements of certain type reside in separate blocks,
// TODO: Should be extended to get block data, BCs, etc.
// and prepare internal data structure properly based on these information
PNTMesh::pntMesh::pntMesh(const meshBase *imb, int dim, int nBlk,
...
std::vector<int> PNTMesh::pntMesh::getElmConn(int id, VTKCellType vct) const
{
...
std::vector<int>
PNTMesh::pntMesh::getPntConn(std::vector<int> &ci, elementType et, int eo) const
{
  std::vector<int> co;
  co = ci;
...
VTKCellType PNTMesh::pntMesh::getVtkCellTag(elementType et, int order) const
{
