...
std::mutex LocalTraceBufferPool::_singletonMutex;
...
int LocalTraceBufferPool::insert(int traceType, const char* msg)
{
  ...
  _myBuffer[myInsertPos%TRACE_BUFFER_SIZE].threadId =
...
: _freeBufferSemaphore(TRACE_BUFFER_SIZE), _fullBufferSemaphore(0)
{
...
{
  _incrementMutex.lock();   // lock access to counters
  unsigned long mypos = ++pos;
  _incrementMutex.unlock(); // release lock
  return mypos;
}
