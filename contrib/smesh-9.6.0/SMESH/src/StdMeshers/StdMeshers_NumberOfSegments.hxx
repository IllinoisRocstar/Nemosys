...
public:
  ...
  const std::vector<double>& BuildDistributionExpr( const char*, int, int ) noexcept(false);
  const std::vector<double>& BuildDistributionTab( const std::vector<double>&, int, int ) noexcept(false);
