...
KMeans::KMeans(int k, int max_iterations, bool verbose)
    : num_clusters_(k), max_iterations_(max_iterations), verbose_(verbose) {}
...
bool KMeans::assign() {
  ...
  for (auto &point : points_) {
    ...
    if (verbose_) {
      cout << "Assigned point " << point << " to cluster: " << new_cluster
           << endl;
    }
  }
  return changed;
