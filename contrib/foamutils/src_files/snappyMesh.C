...
snappyMesh::snappyMesh() { std::cout << "snappyMesh generated!" << std::endl; }

snappyMesh::~snappyMesh() { std::cout << "snappyMesh destroyed!" << std::endl; }

Foam::fvMesh* snappyMesh::generate(
    const std::unique_ptr<Foam::Time>& runTime,
    const std::unique_ptr<Foam::dictionary>& snappyMshDict_,
    const std::unique_ptr<Foam::fvMesh>& fmesh_, const bool& write) {
  const bool overwrite = true;
  const bool checkGeometry = false;
  const bool surfaceSimplify = false;
  const bool dryRun = false;

  // In future, fmesh_ will replace this reading part
  ...
  IOobject dictIO =
      IOobject::selectIO(IOobject(dictName, runTime->system(), mesh,
                                  IOobject::NO_READ, IOobject::NO_WRITE),
                         "");

  const IOdictionary meshDict(dictIO, snappyMshDict_.get());
  ...
  const scalar mergeDist = getMergeDistance(
      mesh, meshRefinement::get<scalar>(meshDict, "mergeTolerance", dryRun),
      dryRun);
  ...
  const word setFormat(
      meshDict.getOrDefault<word>("setFormat", vtkSetWriter<scalar>::typeName));
  const autoPtr<writer<scalar>> setFormatter(writer<scalar>::New(setFormat));

  const scalar maxSizeRatio(meshDict.getOrDefault<scalar>("maxSizeRatio", 100));
  ...
  if (Pstream::parRun()) {
    ...
    IOdictionary* dictPtr = new IOdictionary(IOobject::selectIO(
        IOobject(decompositionModel::canonicalName, runTime->system(), *runTime,
  ...
  if (surfaceSimplify) {
    ...
    IOdictionary foamyHexMeshDict(
        IOobject("foamyHexMeshDict", runTime->system(), *runTime,
    ...
    const dictionary& motionDict = foamyHexMeshDict.subDict("motionControl");

    const dictionary& shapeControlDict =
        motionDict.subDict("shapeControlFunctions");

    // Calculate current ratio of hex cells v.s. wanted cell size
    const scalar defaultCellSize = motionDict.get<scalar>("defaultCellSize");
    ...
    surfacesPtr = createRefinementSurfaces(
        allGeometry, conformationDict, shapeControlDict,
        refineDict.getOrDefault("gapLevelIncrement", 0),
        initialCellSize / defaultCellSize);

    if (write) profiling::writeNow();
  } else {
    surfacesPtr.reset(new refinementSurfaces(
        allGeometry,
        meshRefinement::subDict(refineDict, "refinementSurfaces", dryRun),
        refineDict.getOrDefault("gapLevelIncrement", 0), dryRun));
  ...
  if (checkGeometry) {
      ...
      forAll(regNames, regioni) {
        ...
        if (patchInfo.set(globalRegioni)) {
          patchTypes[geomi][regioni] =
              meshRefinement::get<word>(patchInfo[globalRegioni], "type",
                                        dryRun, keyType::REGEX, word::null);
        } else {
    ...
    // Check geometry
    allGeometry.checkGeometry(maxSizeRatio,  // max size ratio
  ...
  shellSurfaces shells(
      allGeometry,
      meshRefinement::subDict(refineDict, "refinementRegions", dryRun), dryRun);
  ...
  if (write) {
    meshRefiner.write(meshRefinement::debugType(
...

  {
    ...
    if (!dryRun) {
      Info << setf(ios_base::left) << setw(6) << "Patch" << setw(20) << "Type"
           << setw(30) << "Region" << nl << setw(6) << "-----" << setw(20)
           << "----" << setw(30) << "------" << endl;
    }
    ...
    forAll(surfaceGeometry, surfi) {
      ...
      if (fzNames.empty()) {
        ...
        forAll(regNames, i) {
          ...
          if (!dryRun) {
            Info << setf(ios_base::left) << setw(6) << patchi << setw(20)
                 << pbm[patchi].type() << setw(30) << regNames[i] << nl;
          }
        ...
        forAll(regNames, i) {
          ...
          // Add slave side patch
          {
            ...
            if (!dryRun) {
              Info << setf(ios_base::left) << setw(6) << patchi << setw(20)
                   << pbm[patchi].type() << setw(30) << slaveName << nl;
            }
  ...
  if (wantRefine) {
    ...
    refineDriver.doRefine(refineDict, refineParams, snapParams,
                          refineParams.handleSnapProblems(), mergeType,
                          motionDict);
  ...
  if (wantSnap) {
    ...
    snapDriver.doSnap(snapDict, motionDict, mergeType, curvature, planarAngle,
                      snapParams);
  ...
  if (wantLayers) {
    ...
    // Layer addition parameters
    const layerParameters layerParams(layerDict, mesh.boundaryMesh(), dryRun);
    ...
    layerDriver.doLayers(layerDict, motionDict, layerParams, mergeType,
                         preBalance, decomposer, distributor);
  ...
  Info << "Finished meshing in = " << runTime->elapsedCpuTime() << " s."
       << endl;

  // Copy fvMesh now and return that
  ...
  auto* fm = new Foam::fvMesh(
      IOobject(Foam::fvMesh::defaultRegion, runTime->timeName(), *runTime,
...
label snappyMesh::sizeCoeffToRefinement(
...
autoPtr<refinementSurfaces> snappyMesh::createRefinementSurfaces(
    const searchableSurfaces& allGeometry, const dictionary& surfacesDict,
    const dictionary& shapeControlDict, const label gapLevelIncrement,
  ...
  forAll(allGeometry.names(), geomi) {
    ...
    if (ePtr) {
      ...
      const word scsFuncName = shapeDict.get<word>("surfaceCellSizeFunction");
      ...
      // Surface zones
      surfZones.set(surfi, new surfaceZonesInfo(
                               surface, shapeDict,
        ...
        forAll(regionNames, regioni) {
          if (shapeControlRegionsDict.found(regionNames[regioni])) {
            ...
            const word scsFuncName =
                shapeControlRegionDict.get<word>("surfaceCellSizeFunction");
            const dictionary& scsDict =
  ...
  surfacePtr.reset(new refinementSurfaces(
  ...
  Info << setw(maxLen) << "Region" << setw(10) << "Min Level" << setw(10)
       << "Max Level" << setw(10) << "Gap Level" << nl << setw(maxLen)
       << "------" << setw(10) << "---------" << setw(10) << "---------"
       << setw(10) << "---------" << endl;
  ...
  forAll(rf.surfaces(), surfi) {
    ...
    forAll(regionNames, regioni) {
      ...
      Info << setw(maxLen) << regionNames[regioni] << setw(10)
           << rf.minLevel()[globali] << setw(10) << rf.maxLevel()[globali]
...
void snappyMesh::extractSurface(const polyMesh& mesh, const Time& runTime,
...
label snappyMesh::checkAlignment(const polyMesh& mesh, const scalar tol,
                                 Ostream& os) {
...
scalar snappyMesh::getMergeDistance(const polyMesh& mesh, const scalar mergeTol,
                                    const bool dryRun) {
...
void snappyMesh::removeZeroSizedPatches(fvMesh& mesh) {
...
void snappyMesh::writeMesh(const std::string& msg, const meshRefinement& meshRefiner,
