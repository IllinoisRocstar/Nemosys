...
Foam::label Foam::mergePolyMesh::patchIndex(const polyPatch& p) {
  ...
  forAll(patchNames_, patchi) {
    if (patchNames_[patchi] == pName) {
      ...
      if (word(patchDicts_[patchi].lookup("type")) == pType) {
  ...
  if (nameFound) {
    ...
    Info << "label patchIndex(const polyPatch& p) : "
         << "Patch " << p.index() << " named " << pName << " in mesh "
         << caseName << " already exists, but patch types "
...
Foam::label Foam::mergePolyMesh::zoneIndex(DynamicList<word>& names,
                                           const word& curName) {
  forAll(names, zoneI) {
...
void Foam::mergePolyMesh::addMesh(const polyMesh& m) {
  ...
  forAll(p, pointi) {
    ...
    renumberPoints[pointi] =
        meshMod_.setAction(polyAddPoint(p[pointi],  // Point to add
  ...
  forAll(c, celli) {
    ...
    renumberCells[celli] =
        meshMod_.setAction(polyAddCell(-1,     // Master point
                                       -1,     // Master edge
                                       -1,     // Master face
                                       -1,     // Master cell
  ...
  forAll(f, facei) {
    ...
    if (debug) {
      ...
      if (min(newFace) < 0) {
        FatalErrorInFunction << "Error in point mapping for face " << facei
                             << ".  Old face: " << curFace
                             << " New face: " << newFace << abort(FatalError);
      }
    }
    ...
    renumberFaces[facei] =
        meshMod_.setAction(polyAddFace(newFace, newOwn, newNei, -1, -1, -1,
                                       false, newPatch, newZone, newZoneFlip));
  }
}

void Foam::mergePolyMesh::merge() {
  Info << "patch names: " << patchNames_ << nl << "patch dicts: " << patchDicts_
       << nl << "point zone names: " << pointZoneNames_ << nl
  ...
  if (patchNames_.size() != boundaryMesh().size()) {
    ...
    for (; patchi < patchNames_.size(); patchi++) {
      ...
      newPatches[patchi] =
          (polyPatch::New(patchNames_[patchi], dict, patchi, oldPatches).ptr());
    }
  ...
  if (pointZoneNames_.size() > pointZones().size()) {
    ...
    for (label zoneI = nZones; zoneI < pointZoneNames_.size(); zoneI++) {
      pointZones().set(zoneI, new pointZone(pointZoneNames_[zoneI], labelList(),
                                            zoneI, pointZones()));
    }
  }
  if (cellZoneNames_.size() > cellZones().size()) {
    ...
    for (label zoneI = nZones; zoneI < cellZoneNames_.size(); zoneI++) {
      cellZones().set(zoneI, new cellZone(cellZoneNames_[zoneI], labelList(),
                                          zoneI, cellZones()));
    }
  }
  if (faceZoneNames_.size() > faceZones().size()) {
    ...
    for (label zoneI = nZones; zoneI < faceZoneNames_.size(); zoneI++) {
      faceZones().set(zoneI, new faceZone(faceZoneNames_[zoneI], labelList(),
                                          boolList(), zoneI, faceZones()));
    }
  }
