...
enum algorithmDefinition { SPT_SWP_SLV_CPS, CPS_SWP_SLV_SPT, SLV_CPS_SWP_SPT };
#ifdef PARALLEL
enum loadBalanceAlgorithm { DEFAULT_ALGORITHM, METIS_ALGORITHM };
#endif
// -------------------------------------------------------------------
// -------------------------------------------------------------------
class MADLIB_EXPORT MeshAdapter {
 public:
  ...
  void addCallback(CBFunction CB, void *userData, CBFunc_move CB_move = 0,
                   void *userData_move = 0);
...
//     void setDataExchanger( MDB_DataExchanger* dataExch);
  ...
  void registerVData(std::string name,
 ...
 public:
  ...
  void collapseEdgeBrute(
      pEdge e);  // use it only if you know what your are doing
  ...
  //    bool moveVertices (std::multiset<vDisplacement,vDisplacementLess>&
  //    vDisps);
  ...
  //   subAdaptation:    allow sliver elimination and mesh optimization if
  //   necessary qualityThreshold: at which quality do we optimize the mesh (if
  //   allowed) chi:              stiffness alteration coefficient (-1 = none)
  //   meshIsCavity:     true = elastic computation on whole mesh
  //   cavityThickness:  nb layers of elements if mesh is not the cavity
  void moveObjectsAndReposition(double t, double dt, bool subAdaptation = true,
                                double qualityThreshold = 0., double chi = -1.,
 ...
 public:
  ...
  void getModificationsInfo(int *nSplit, int *nColl, int *nSwap,
                            double *cpuSplit, double *cpuColl, double *cpuSwap,
  ...
  bool checkTheMesh(int verbose = 1, std::ostream &out = std::cout,
